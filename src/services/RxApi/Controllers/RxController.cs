        ...
        foreach (var planet in planets)
        {
            ...
            if (currentRxPeriod != null)
            {
                ...
                var nextShadowSurpass = await _context.PlanetPositions
                    .Where(position =>
                        position.Name == planet && position.Longitude > currentRxPeriod.StartPosition.Longitude &&
                        position.Time > currentRxPeriod.EndPosition.Time)
                    .OrderBy(position => position.Time).FirstOrDefaultAsync();

                if (nextShadowSurpass != null) periodsDTO.ExitsShadow = nextShadowSurpass;
            }
            ...
            if (nextRxPeriod != null) periodsDTO.After = nextRxPeriod;
            ...
            if (pastRxPeriod != null) periodsDTO.Previous = pastRxPeriod;
